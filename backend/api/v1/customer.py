...
from backend.utils.jwt import get_customer_from_refresh
...
def update_user(
    ...
    current_user: Customer = Depends(get_customer_from_refresh),
...
def delete_own_account(db:Session=Depends(get_db),current_user:Customer=Depends(get_customer_from_refresh)):
